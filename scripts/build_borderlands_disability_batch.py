#!/usr/bin/env python3
"""Reproduce the bounded borderlands/disability packet without production writes."""
import argparse
...
OUT = ROOT / 'data/extension-2026/borderlands-disability-08'
COVERAGE = ROOT / 'data/extension-2026/digital-web-07/coverage-current.json'
FIELDS = ('disability', 'borderlands_history')
...
def build(recipe=None):
    ...
    for a in r['prior_work_annotations']:
        ...
        e.update({k: deepcopy(a[k]) for k in ('author_ids', 'publication_year_observation', 'candidate_navigation_ids')})
    for s in r['sources']:
        path = OUT / s['capture']
    ...
    for e in r['people'] + r['works'] + r['versions']:
    ...
    def add(row, source, locator, status, support, limit, basis):
        row.update(basis=basis, valid_time=None,
                   ...
                   history=[dict(date=r['cutoff'], action='proposed', reason='Borderlands/disability continuation and contributor review')],
    ...
    for w in r['works'] + [dict(a, id=a['work_id']) for a in r['prior_work_annotations']]:
        for pid in w['author_ids']:
            add(dict(id='claim:borderdis08:authored:'+w['id'].split(':',1)[1]+':'+pid.split(':',1)[1],
    ...
    for v in r['versions']:
        add(dict(id='claim:borderdis08:realizes:'+v['id'].split(':')[-1], subject=v['id'],
                 predicate='realizes', object=v['work_id'], attributed_to=v['source_record_id']),
            v['source_record_id'], 'Version identification in cover or p. 17 note 1', 'metadata_checked',
    ...
    batch = dict(schema_version='0.2', fixture_only=True, status='staging_only', production_graph_imports=0,
                 ...
                 derivation=dict(prior_packet=r['prior_packet'], prior_claim_ids=r['prior_claim_ids'],
                                 note='Three inherited works/claims overlap breadth-01; not additional unique discoveries.'))
    ...
    mapping = {s['id']: 'borderdis08_' + s['id'].split(':')[-1] for s in sources}
    ...
    for field in FIELDS:
        ...
        for w in ws:
            ...
            strands.append(dict(id='borderdis08_'+w['id'].split(':',1)[1], title=w['label'],
        ...
        node = dict(id=field, **scope, entry_kind='group', entry_type='Research field',
                    layer='enduring_fields_and_genres', period=None, hunt_core_paradigm=False,
        ...
        drafts.append(dict(candidate_id=field, status='needs_review', proposed_disposition='separate_entry',
                           node=node, production_import=False, acceptance_requirements=[
                               ...
                               'Preserve translation, manuscript and reception chronology',
                               'Do not treat the separate-entry proposal as a completed field review']))
    coverage = deepcopy(read(COVERAGE))
    coverage.update(derived_from=str(COVERAGE.relative_to(ROOT)), supplement_packet=str((OUT/'batch.json').relative_to(ROOT)))
    for row in coverage['field_candidates']:
        ...
        row['research_bin_note'] = 'Bins date witnessed interventions; expanded translation/reception does not establish an argument origin.'
    ...
    return {'batch.json':batch,'review-actions.json':actions,'source-proposals.json':source_proposals,
            'entry-proposals.json':drafts,'coverage-current.json':coverage,'summary.json':summary}
...
def main():
    ...
    if args.check:
        ...
        files += [Path(__file__).resolve(),ROOT/'tests/test_borderlands_disability_batch.py']
